...
/**
 * Separates the Entity Nodes from the remaining nodes, stores the attribute information in a hashmap, and renders them.
 */

public class EntityGroup extends NodeGroup{
    Logger logger = Logger.getLogger("EntityGroup");

	HashMap<Double, String> entityHashmap;
	static HashMap<Double, List<String>> entityValuesHashmap;
	static String nameTag = "org.gk.render.RenderableEntity";
	
	/**Separates the Entity Nodes from the remaining nodes
	 * 
	 * @param allNodes XML ELement containing the Nodes of the Network
	 */
	public EntityGroup(Element allNodes) {
		super(allNodes,nameTag);
		entityHashmap = new HashMap<Double, String>();
		entityValuesHashmap = new HashMap<Double, List<String>>();
	}

	/**
	 * Builds a HashMap containing the information of the ID, Bounds, Position of the Entity Nodes
	 * Builds a HashMap containing the information of DisplayName and Node Id
	 */
	public void buildHashMap() {
	    for (int i = 0; i < noofnodes ; i++){
			Node RenderableNode = nodes.item(i);
			Element elm = (Element) RenderableNode;
			String id1 = elm.getAttribute("id");
			logger.info("Handling id: " + id1);
			...
			if(RenderableNode.hasChildNodes()){
				...
				entityHashmap.put(reactomeIdNo, DisplayName);
			} else {
				if (entityHashmap.containsKey(reactomeIdNo)) {
					DisplayName = entityHashmap.get(reactomeIdNo);
				} else {
			...
			entityValuesHashmap.put(idNo, attributes);
			BoundsHashmap.put(idNo, Attributeparser.getStringAttributes("bounds"));
			
		}
	}
	
	/**Renders the Entity Nodes on the Context
	 * 
	 * @param context The Context2d Object on which the Nodes are rendered
	 */
	public void update(Context2d context) {
		Iterator<Double> iterator = entityValuesHashmap.keySet().iterator();
		while(iterator.hasNext()) {
			Double key = (Double) iterator.next();
			List<String> attributes = entityValuesHashmap.get(key);
			EntityNode entity = new EntityNode(attributes.get(0), attributes.get(2), attributes.get(3), attributes.get(4));
			entity.draw(context);
			int fontSize = 13;
